...
OBS WebSocket 5.x と連携して、**特定ソースを揺らす／拡大縮小する**といったエフェクトをかけられます。

- **前提**
  - OBS側に `obs-websocket` 5.x を導入し、有効化しておく
  - ポート・パスワードを確認（デフォルト: ポート `4455`）
- **設定場所**
  - `/overlay` のテストモードパネル → 「OBS WebSocket API」 セクション
  - または `/overlay` ページの設定パネル内「OBS WebSocket API設定（ソースレイヤー操作）」
- **主な設定項目**
  - `有効化` チェックボックス
  - `ホスト`（例: `localhost`）
  - `ポート`（例: `4455`）
...
> **補足:**
> OBS WebSocketへの接続／認証エラーやタイムアウトが発生しても、ページが真っ黒にならないように、Promiseは必ず解決し UI はそのまま描画されるように実装されています（ログには警告が出ます）。
...
- **浅い共通層**: `src/{api,components,hooks,lib}`。API クライアントの実体は `utils/twitchApi.ts`、`api/index.ts` から再エクスポート。
