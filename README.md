...
初回は `npx` が静的サーバ用パッケージを取りに行くため **インターネット**が必要です。ポート変更は環境変数 **`OVERLAY_PORT`** を設定してから bat を実行（例: `set OVERLAY_PORT=8080`）。トラブル時・細かい説明は同梱の **`README-配布.txt`**（`release` フォルダ内）を参照してください。

### 配布パッケージを作る人

1. リポジトリを用意し、**`npm ci`**（または Windows の **`install.bat`**）。
...
`package-lock.json` に固定された依存のみ入ります（ルートの `.npmrc` でサプライチェーン対策を有効化）。

```bash
npm ci
```

Windows では **`install.bat`** でも同様です。npm は **11.10 以上**を推奨（`min-release-age`）。確認: `npm --version`
...
- **設定画面のレイアウト**:
  - **タブ**: 「配信者側」「ユーザー側」「自動返信設定」の3タブ。自動返信設定タブ内は「配信者側」「ユーザー側」のサブタブで自動返信のみを編集可能。
...
- **設定**:
  - テストモード中も設定画面と同様に、**配信者側**・**ユーザー側**・**自動返信設定**の3タブで各種設定を編集可能。
  - 配信者側: HP・攻撃・回復・リトライなど。ユーザー側: PvPモード、攻撃モード（配信者vs視聴者のみ／両方）、視聴者同士攻撃コマンド・ダメージ（「両方」選択時のみ表示）、HP確認・回復コマンドなど。自動返信設定: 配信者側・ユーザー側の自動返信のオンオフとメッセージをサブタブで編集。
...
- **保存先**: 設定の保存先は **JSON ファイルのみ**（`public/config/overlay-config.json`）。ローカルストレージには保存せず、設定のありかは1箇所に統一しています。
- **読み込み**: 起動時・再読み込み時は **JSON ファイル → 見つからない場合はデフォルト設定** の順で読み込みます。
- **設定画面のボタン**:
  - **設定を保存**: 開発環境では上記 JSON ファイルに上書き保存。本番環境では JSON ファイルがダウンロードされるので、手動で `public/config/overlay-config.json` などに配置してください。
  - **設定を再読み込み（ファイル選択）**: 手元の JSON ファイルを選んでフォームに反映。保存するには「設定を保存」を押します。
  - **JSONファイルから読み込み**: サーバー上の `public/config/overlay-config.json` の内容でフォームを上書きします（ディスクの最新内容を反映したいときに使用）。
  - **リセット**: デフォルト設定でフォームを上書きします。
...
| `npm run package:release` | 配布用 `release/` を生成（[配布パッケージを作る人](#配布パッケージを作る人)） |
...
- **`.npmrc`**: `ignore-scripts` / `min-release-age=7`（npm 11.10+）など（[Qiita 記事](https://qiita.com/masato_makino/items/516ca6f8a8b497131602) を参考）
- **`npm ci`**: lockfile のみから再現インストール（`npm install` は新規解決で lock がずれるため非推奨）
- **Dependabot**: `.github/dependabot.yml`（cooldown 7 日）
- **CI**: `.github/workflows/npm-security.yml`（`npm audit`、PR 時 dependency review）
...
├── constants/           # オーバーレイ用定数（techniqueEffectKinds, comboTechniqueNames, hpGaugeOverlay 等）
├── hooks/               # カスタムフック（Twitch / オーバーレイ / チャット等）
├── lib/                 # logger, TanStack Query の queryClient / queryKeys
├── pages/               # ルートページ（OverlayPage, OAuthCallbackPage 等）
├── types/               # overlay.ts / twitch.ts
├── config/              # auth, admin
├── utils/               # twitchApi, twitchChat, overlayConfig 等
├── App.tsx
└── main.tsx             # QueryClientProvider + DebugLog をマウント
```
