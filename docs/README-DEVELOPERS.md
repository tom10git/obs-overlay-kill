...
このドキュメントは「リポジトリを clone して開発・ビルド・課金基盤（Supabase/Stripe）まで扱う人」向けです。
...
Secret 類（Stripe秘密鍵、Twitch Client Secret 等）は `VITE_` に置かず **Supabase secrets** へ。

- 詳細: `docs/SECURITY.md`
- 課金・招待コード: `docs/BILLING.md`
...
| `scripts/deploy-billing-functions.bat` | 課金基盤 | `npx supabase functions deploy ...` を一括実行 | `npx supabase login` が必要 |
| `scripts/create-invites.bat` | 招待コード運用 | JSON を元に招待コードを一括登録 | **`BILLING_ADMIN_SECRET` は配布しない** |
